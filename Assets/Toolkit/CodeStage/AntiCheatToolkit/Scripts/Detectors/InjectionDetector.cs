...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class InjectionDetector : ActDetectorBase
	{
		...
		private void StartDetectionInternal(UnityAction callback)
		{
			...
			{
				Debug.LogWarning(Constants.LOG_PREFIX + COMPONENT_NAME + " is not enabled in Anti-Cheat Toolkit Settings!\nPlease, check readme.pdf for details.", this);
				DisposeInternal();
				return;
			}
#if !DEBUG_NORMAL
			...
			{
				Debug.LogWarning(Constants.LOG_PREFIX + COMPONENT_NAME + " does not work in editor (check readme.pdf for details).", this);
				DisposeInternal();
				return;
			}
#else
			Debug.LogWarning(Constants.LOG_PREFIX + COMPONENT_NAME + " works in debug mode. There WILL BE false positives in editor, it's fine!", this);
#endif
#endif
		...
		public static InjectionDetector Instance
		{
			get
			{
				Debug.LogError(Constants.LOG_PREFIX + COMPONENT_NAME + " is not supported on selected platform!");
				return null;
			}
		}
