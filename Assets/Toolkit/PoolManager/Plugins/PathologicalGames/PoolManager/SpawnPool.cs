...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
		...
		public Transform DespawnHolderTF;
		public Transform SpawnHolderTF;
        ...
        public RectTransform SpawnRT(string prefabName) {
            Transform prefab = this.prefabs[prefabName];
            RectTransform _rt =(RectTransform)this.Spawn(prefab);
            _rt.anchoredPosition3D = _rt.anchoredPosition;
            return _rt;
        }
		...
		public Transform SpawnToDefault (string prefabName) {
			Transform prefab = this.prefabs[prefabName];
			return this.Spawn(prefab, SpawnHolderTF);
		}
        ...
        public RectTransform SpawnRT(string prefabName, RectTransform parent) {
            Transform prefab = this.prefabs[prefabName];
            RectTransform _rt = (RectTransform)this.Spawn(prefab, parent);
            _rt.anchoredPosition3D = _rt.anchoredPosition;
            return _rt;
        }
		...
		public void DespawnToDefault (Transform instance) {
			instance.SetParent(DespawnHolderTF);
			this.Despawn(instance);
		}
		...
		public void DespawnAllToDefault () {	
			var spawned = new List<Transform>(this._spawned);
			for (int i = 0; i < spawned.Count; i++) {
				spawned[i].SetParent(DespawnHolderTF);
				this.Despawn(spawned[i]);
			}
		}

        public void DespawnAll(Transform _newParent) {
            var spawned = new List<Transform>(this._spawned);
            for (int i = 0; i < spawned.Count; i++) {
                spawned[i].SetParent(_newParent);
                this.Despawn(spawned[i]);
            }
                
        }

		public void DespawnAllToDefaultIn (Transform _holder) {
			List<Transform> _list = new List<Transform>();
			for (int i = 0; i < _holder.childCount; i++) {
				_list.Add(_holder.GetChild(i));
			}
			for (int i = 0; i < _list.Count; i++) {
				if (this.IsSpawned(_list[i])) {
					this.DespawnToDefault(_list[i]);
				}
			}
			_list.Clear();
		}
