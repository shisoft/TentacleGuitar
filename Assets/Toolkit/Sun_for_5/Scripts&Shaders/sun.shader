	...
	float3 RGBtoHSL(in float3 RGB)
  {
    ...
    return float3(HCV.x, S, L);
        ...
        fixed4 frag (v2f i) : SV_Target
        {
          ...
          float glow =pow( 2.0-dot(-normalize((mul((float4x4)_Object2World, float4(0.0,0.0,0.0,1.0))-_WorldSpaceCameraPos)), normalize(mul((float3x3)_Object2World,i.normal))),(_Contour*4.0+2.0));
