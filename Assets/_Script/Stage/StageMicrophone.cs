namespace TentacleGuitarUnity {
	...
	public class StageMicrophone : MonoBehaviour {
		...
		public static void MicrophoneStart (int length) {
			if (!Main)
				return;
			Main.Source.clip = Microphone.Start(null, true, length, sampleRate);
		...
		public static float[] GetSpectrumData (int bins) {
			if (!Main || !IsReady) {
				return null;
			}
			float[] f = new float[bins];
			Main.Source.GetSpectrumData(f, 0, FFTWindow.BlackmanHarris);
			return f;
		}
